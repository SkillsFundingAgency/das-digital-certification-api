...
{
    public class WhenGettingUserIdentity
    {
        [Test, MoqAutoData]
        public async Task And_QueryIsSuccessful_Then_ReturnOk(
            Guid userId,
            GetUserIdentityQueryResult queryResult,
            [Frozen] Mock<IMediator> mediator,
            [Greedy] UsersController controller)
        {
            // Arrange
            mediator
                .Setup(m => m.Send(It.Is<GetUserIdentityQuery>(q => q.UserId == userId), It.IsAny<CancellationToken>()))
                .ReturnsAsync(queryResult);

            // Act
            var result = await controller.GetUserIdentity(userId);
            ...
            [Greedy] UsersController controller)
        {
            // Arrange
            mediator
                .Setup(m => m.Send(It.IsAny<GetUserIdentityQuery>(), It.IsAny<CancellationToken>()))
