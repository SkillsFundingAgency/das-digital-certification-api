...
namespace SFA.DAS.DigitalCertificates.Api.UnitTests.Controllers.Users
{
    public class WhenCreatingUserAction
    {
        ...
        public async Task And_ValidRequest_Then_MediatorIsCalledAndReturns200()
        {
            ...
            var response = new CreateUserActionCommandResponse
            {
            ...
            // Act
            var result = await _sut.CreateUserAction(userId, command);

            // Assert
            _mediatorMock.Verify(m => m.Send(It.Is<CreateUserActionCommand>(c => c.UserId == userId), It.IsAny<CancellationToken>()), Times.Once);
